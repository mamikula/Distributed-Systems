...
arr = [_ for _ in range(1000000)]
diction = {_: _ for _ in range(1000000)}
arr_ref = ray.put(arr)
dict_ref = ray.put(diction)

def local_process_arr(arr):
    print(sum(arr))

@ray.remote
def remote_process_arr(arr_ref):
    print(sum(arr_ref))


def local_process_dict(diction):
    print(sum(diction.values()))


@ray.remote
def remote_process_dict(dict_ref):
    print(sum(dict_ref.values()))

print("Array local task")
cProfile.run("local_process_arr(arr)")

print("Array remote task")
cProfile.run("ray.get(remote_process_arr.remote(arr_ref), timeout=1)")

print("###########################################")

print("Dictionary local task")
cProfile.run("local_process_dict(diction)")

print("Dictionary remote task")
cProfile.run("ray.get(remote_process_dict.remote(dict_ref), timeout=1)")
